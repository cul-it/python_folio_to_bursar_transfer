...
class TransferFineAction:
    def __init__(self, conf, folio_connection, trans_active):
        ...
        logger.info("Initializing TransferFineAction with settings: %s", conf["name"])
        process_active = conf["process_active"] if trans_active in conf else True
        self.__do_transfer = not (str(process_active).lower() == "false" or str(trans_active).lower() == "false")
    ...
    def check(self, fine):
        ...
        :return: fine.
        """
        logger.info("Checking transfer conditions for fine ID: %s", fine.get("id"))
        if self.__conf["name"] not in fine:
            fine[self.__conf["name"]] = {}
        if fine["amount"] <= 0:
            logger.warning("Transfer not allowed for non-positive amounts. Fine ID: %s", fine.get("id"))
            fine[self.__conf["name"]]["check"] = {}
            fine[self.__conf["name"]]["check"]["allowed"] = False
        else:
            logger.debug("Transfer allowed for positive amounts. Fine ID: %s", fine.get("id"))
            body = {"amount": fine["amount"]}
            url = f"/accounts/{fine['id']}/check-transfer"
            logger.debug("Checking transfer action for fine ID: %s with URL: %s", fine["id"], url)
            try:
                check_response = self.__connector.post_request(url, body)
                fine[self.__conf["name"]]["check"] = check_response
                if check_response["allowed"]:
                    logger.info("Transfer check passed for fine ID: %s", fine["id"])
                else:
                    logger.warning("Transfer check failed for fine ID: %s", fine["id"])
                return fine
            except Exception as e:
                logger.error("Error during transfer check for fine ID: %s. Error: %s", fine["id"], e)
                raise
    ...
    def execute(self, fine):
        ...
        if self.__do_transfer:
            logger.debug("Processing transfer for fine ID: %s with URL: %s", fine["id"], url_2)
            ...
            try:
                ...
                fine[self.__conf["name"]]["process"] = {"message": "Transfer processed successfully"}
                logger.info("Transfer processed successfully for fine ID: %s", fine["id"])
            except Exception as e:
                logger.error("Error processing transfer for fine ID: %s. Error: %s", fine["id"], e)
                raise
        else:
            logger.warning("Transfer not processed for fine ID: %s", fine["id"])
