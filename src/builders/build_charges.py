...
class BuildCharges:
    ...
    def __get_outstanding_fines_all(self):
        ...
        charge_days_outstanding = self.__settings.get("charge_days_outstanding", 0)
        ...
        logger.info("Reported record count: %d", self.__filter_data['reportedRecordCount'])
    ...
    def __get_material_data(self, fines):
        ...
        for f in fines:
            if 'materialTypeId' not in f:
                ...
                logger.debug("Material data added for fine: %s", f['materialTypeId'])
