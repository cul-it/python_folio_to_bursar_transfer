...
class SftpUploader:
    ...
    def __init__(self, env_key):
        ...
        load_dotenv()
        self.__ftp_remote_path = os.getenv(f"{env_key}_REMOTE_PATH")
        ...
        if os.getenv(f"{env_key}_PASSWORD").upper() == 'CERTIFICATE':
            if os.getenv(f"{env_key}_CERTIFICATE_LOCATION").upper() == 'LOCAL':
                # Local certificate from the local file system
                with open(os.getenv(f"{env_key}_CERTIFICATE_PATH"), 'r', encoding='utf-8') as file:
                    certificate = file.read()
            elif 'AWS' in os.getenv(f"{env_key}_CERTIFICATE_LOCATION").upper():
        ...
        if not os.getenv(f"{env_key}_KNOW_HOSTS_LOCATION"):
            ...
            if os.getenv(f"{env_key}_KNOW_HOSTS_LOCATION").upper() == 'LOCAL':
                # Local certificate from the local file system
                with open(os.getenv(f"{env_key}_KNOW_HOSTS_PATH"), 'r', encoding='utf-8') as file:
        ...
        if os.getenv(f"{env_key}_PASSWORD").upper() == 'CERTIFICATE':
            self.__ssh_client.connect(
                hostname=os.getenv(f"{env_key}_HOST"),
                port=os.getenv(f"{env_key}_PORT"),
